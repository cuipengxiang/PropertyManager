...
- (void)viewDidLoad
{
    ...
    NSString *urlString=[NSString stringWithFormat:@"%@", @"http://219.146.138.106:9292/ourally/app/property/sys/sysLogin.do"];
...
{
    NSString *urlString = [NSString stringWithFormat:@"http://219.146.138.106:9292/ourally/app/property/message/getMessageList.do?userId=%@", [[NSUserDefaults standardUserDefaults] objectForKey:@"userid"]];
    NSURL *url=[NSURL URLWithString:urlString];
	NSURLRequest *request=[NSURLRequest requestWithURL:url];
	[self.mainWebView loadRequest:request];
}
