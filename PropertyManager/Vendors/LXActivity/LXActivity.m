...
- (id)initWithTitle:(NSString *)title delegate:(id<LXActivityDelegate>)delegate cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitle:(NSString *)otherButtonTitle ShareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray;
{
    ...
    if (self) {
        ...
        [self creatButtonsWithTitle:title cancelButtonTitle:cancelButtonTitle otherButtonTitle:otherButtonTitle shareButtonTitles:shareButtonTitlesArray withShareButtonImagesName:shareButtonImagesNameArray];
    }
    return self;
}
...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitle:(NSString *)otherButtonTitle shareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray
{
    //初始化
    self.isHadTitle = NO;
    ...
    if (shareButtonImagesNameArray) {
        if (shareButtonImagesNameArray.count > 0) {
            ...
            for (int i = 1; i < shareButtonImagesNameArray.count+1; i++) {
                ...
                [shareButton setBackgroundImage:[shareButtonImagesNameArray objectAtIndex:i-1] forState:UIControlStateNormal];
                //有Title的时候
    ...
    if (otherButtonTitle) {
        ...
        UIButton *otherButton = [self creatOtherButtonWith:otherButtonTitle];
        otherButton.tag = self.postionIndexNumber;
        [otherButton addTarget:self action:@selector(didClickOnOtherButton) forControlEvents:UIControlEventTouchUpInside];
        
        //当没title destructionButton otherbuttons时
        if (self.isHadTitle == NO && self.isHadShareButton == NO) {
            self.LXActivityHeight = self.LXActivityHeight + otherButton.frame.size.height+(2*BUTTON_INTERVAL_HEIGHT);
        }
        //当有title或destructionButton或otherbuttons时
        if (self.isHadTitle == YES || self.isHadShareButton == YES) {
            [otherButton setFrame:CGRectMake(otherButton.frame.origin.x, self.LXActivityHeight, otherButton.frame.size.width, otherButton.frame.size.height)];
            self.LXActivityHeight = self.LXActivityHeight + otherButton.frame.size.height+BUTTON_INTERVAL_HEIGHT;
        }
    ...
    if (cancelButtonTitle) {
        ...
        //当没title destructionButton otherbuttons时
        if (self.isHadTitle == NO && self.isHadShareButton == NO && self.isHadOtherButton == NO) {
            self.LXActivityHeight = self.LXActivityHeight + cancelButton.frame.size.height+(2*BUTTON_INTERVAL_HEIGHT);
        }
        //当有title或destructionButton或otherbuttons时
        if (self.isHadTitle == YES || self.isHadShareButton == YES || self.isHadOtherButton == YES) {
...
- (void)didClickOnOtherButton
{
    if (self.delegate) {
        if ([self.delegate respondsToSelector:@selector(didClickOnOtherButton:)] == YES) {
